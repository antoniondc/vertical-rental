...
class SaleRental(models.Model):
    ...
    def _get_rental_display_name(self):
        self.ensure_one()
        return "[{}] {} - {} > {} ({})".format(
            self.partner_id.display_name or "",
            self.rented_product_id.display_name or "",
            self.start_date or "",
            self.end_date or "",
            self._fields["state"].convert_to_export(self.state, self) or "",
        )
    ...
    def _compute_display_name(self):
        for rental in self:
            rental.display_name = rental._get_rental_display_name()

    def name_get(self):
        return [(rental.id, rental._get_rental_display_name()) for rental in self]
    ...
    def cron_send_return_reminders(self):
        ...
        if not template:
            logger.warning(
                "Email template sale_rental.mail_template_rental_return_reminder not found."
            )
            return
